...
const LL: Record<number, string> = { 0: "Bajo", 1: "Medio", 2: "Alto" };
const TT = { backgroundColor: "#1d1d1f", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, fontSize: 12 };
const TL = { color: "#fff", fontSize: 12 };
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={data} margin={{ top: 4, right: 8, left: -24, bottom: 4 }}>
          ...
          <YAxis domain={[0, 2]} ticks={[0, 1, 2]}
            tickFormatter={v => LL[v as number] ?? v} tick={{ fill: "#6e6e73", fontSize: 9 }} />
          <Tooltip contentStyle={TT} labelStyle={TL}
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            formatter={(v: any, n: any) => [LL[v as number] ?? v, String(n)]} />
          <Legend wrapperStyle={{ fontSize: 11, color: "#6e6e73" }} />
          {(["IRN", "ISR", "USA"] as const).map(c =>
            active.has(c) ? <Line key={c} type="stepAfter" dataKey={c} stroke={CC[c]} strokeWidth={2} dot={false} connectNulls /> : null
          )}
        </LineChart>
      </ResponsiveContainer>
    </div>
...
const FIRMS_DAILY = [
  { dia: "26 may", hotspots: 892,  frp_total: 18450 },
  { dia: "27 may", hotspots: 1203, frp_total: 24800 },
  { dia: "28 may", hotspots: 1467, frp_total: 31200 },
  { dia: "29 may", hotspots: 1834, frp_total: 38900 },
  { dia: "30 may", hotspots: 956,  frp_total: 19800 },
];

export function FRPDailyChart() {
  return (
    <ResponsiveContainer width="100%" height={200}>
      <ComposedChart data={FIRMS_DAILY} margin={{ top: 4, right: 44, left: -8, bottom: 4 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
        <XAxis dataKey="dia" tick={{ fill: "#6e6e73", fontSize: 10 }} />
        <YAxis yAxisId="l" tick={{ fill: "#6e6e73", fontSize: 9 }} />
        <YAxis yAxisId="r" orientation="right" tick={{ fill: "#ff9f0a", fontSize: 9 }} />
        <Tooltip contentStyle={TT} labelStyle={TL} />
        <Bar yAxisId="l" dataKey="hotspots" name="Puntos calientes" fill="#ff3b30" opacity={0.7} radius={[3,3,0,0]} />
        <Line yAxisId="r" type="monotone" dataKey="frp_total" name="FRP total (MW)" stroke="#ff9f0a" strokeWidth={2.5} dot={{ fill:"#ff9f0a", r:4 }} />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
...
export function GoldsteinBarChart() {
  return (
    <div>
      <div className="mb-4 text-xs font-mono text-white/25 tracking-widest">ESCALA: −10 (conflicto máximo) → +10 (cooperación máxima)</div>
      <div className="space-y-5">
        {GOLDSTEIN_DATA.map(d => {
          const sc = (v: number) => ((v + 10) / 20) * 100;
          return (
            <div key={d.country}>
              <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="relative h-3 bg-white/5 rounded-full overflow-hidden">
                ...
                <div className="absolute top-1 bottom-1 rounded-full"
                  style={{ left: `${sc(d.min)}%`, width: `${sc(d.max) - sc(d.min)}%`, backgroundColor: d.fill, opacity: 0.22 }} />
                <div className="absolute top-0 bottom-0 w-0.5 rounded-full"
                  style={{ left: `${sc(d.mean)}%`, backgroundColor: d.fill }} />
              </div>
              <div className="flex justify-between text-[9px] font-mono text-white/20 mt-1">
                <span>mín {d.min}</span><span>máx {d.max}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="mt-6 pt-4 border-t border-white/8 text-xs text-white/30 leading-relaxed">
        Israel registra el tono más negativo (−4.87) · promedio histórico Oct 2023 – May 2026
      </div>
    </div>
  );
}
...
const HEATMAP_DATA: Record<string, Record<string, number>> = {
  "Oct 23":{IRN:1,ISR:2,USA:1},"Nov 23":{IRN:1,ISR:2,USA:1},"Dic 23":{IRN:2,ISR:2,USA:1},
  "Ene 24":{IRN:1,ISR:2,USA:1},"Feb 24":{IRN:1,ISR:1,USA:0},"Mar 24":{IRN:2,ISR:2,USA:1},
  "Abr 24":{IRN:2,ISR:2,USA:2},"May 24":{IRN:1,ISR:2,USA:1},"Jun 24":{IRN:1,ISR:1,USA:0},
  "Jul 24":{IRN:1,ISR:1,USA:0},"Ago 24":{IRN:1,ISR:1,USA:1},"Sep 24":{IRN:1,ISR:2,USA:1},
  "Oct 24":{IRN:2,ISR:2,USA:2},"Nov 24":{IRN:2,ISR:2,USA:1},"Dic 24":{IRN:1,ISR:1,USA:1},
  "Ene 25":{IRN:1,ISR:1,USA:0},"Feb 25":{IRN:2,ISR:1,USA:1},"Mar 25":{IRN:2,ISR:2,USA:2},
  "Abr 25":{IRN:1,ISR:1,USA:1},"May 25":{IRN:1,ISR:1,USA:0},"Jun 25":{IRN:0,ISR:1,USA:0},
  "Jul 25":{IRN:0,ISR:1,USA:0},"Ago 25":{IRN:1,ISR:1,USA:1},"Sep 25":{IRN:1,ISR:1,USA:1},
  "Oct 25":{IRN:1,ISR:1,USA:1},"Nov 25":{IRN:1,ISR:2,USA:1},"Dic 25":{IRN:1,ISR:1,USA:1},
  "Ene 26":{IRN:2,ISR:1,USA:1},"Feb 26":{IRN:2,ISR:2,USA:1},"Mar 26":{IRN:2,ISR:2,USA:2},
  "Abr 26":{IRN:2,ISR:2,USA:2},"May 26":{IRN:2,ISR:2,USA:1},
};
  ...
  return (
    <div>
      <div className="grid" style={{ gridTemplateColumns: `56px repeat(${MONTHS.length}, 1fr)`, gap: "2px" }}>
        <div/>
        {MONTHS.map(m => <div key={m} className="text-[7px] font-mono text-white/20 text-center pb-1 leading-tight">{m}</div>)}
        {COUNTRIES.map(c => (
          <>
            <div key={`l-${c}`} className="text-xs font-bold font-mono text-right pr-2 flex items-center justify-end" style={{ color: CC[c] }}>{c}</div>
            {MONTHS.map(m => {
      ...
      {hovered && (
        <div className="mt-2 text-xs text-center font-mono text-white/40">
...
const RADAR_DATA = [
  ...
  { metric:"Velocidad",  KNN:90, LR:95, Ridge:98, NB:99 },
];
...
export function ConflictIntensityChart({ data }: { data: TimelinePoint[] }) {
  ...
  const chartData = data.slice(-90).map(d => {
    ...
    return { date: d.date, intensidad: parseFloat(avg.toFixed(2)) };
  });
  return (
    <ResponsiveContainer width="100%" height={160}>
      <ComposedChart data={chartData} margin={{ top:4, right:8, left:-28, bottom:4 }}>
        ...
        <XAxis dataKey="date" tick={{ fill:"#6e6e73", fontSize:9 }} tickFormatter={d => d.slice(5)} interval={Math.floor(chartData.length/5)} />
        <YAxis domain={[0,2]} ticks={[0,1,2]} tickFormatter={v => LL[v as number] ?? v} tick={{ fill:"#6e6e73", fontSize:9 }} />
        <Tooltip contentStyle={TT} labelStyle={TL} />
        ...
        <Area type="monotone" dataKey="intensidad" name="Intensidad media" stroke="#ff3b30" fill="url(#ig)" strokeWidth={2} />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
